...
from dataloaders.kits_Dataset import KiTS1
...
parser.add_argument('--root_path', type=str, default=r'input your path', help='Name of Dataset')
...
parser.add_argument('--self_max_iteration', type=int, default=13000, help='maximum self-train iteration to train')
...
parser.add_argument('--labelnum', type=int, default=19, help='trained samples')
...
def pre_train(args, snapshot_path):
    ...
    trainset_lab_a = KiTS1(train_data_path, "./Datasets/kits", split=f'train_lab{args.labelnum}', patch_size=patch_size)
    lab_loader_a = DataLoader(trainset_lab_a, batch_size=c_batch_size, shuffle=False, num_workers=0, drop_last=True)

    trainset_lab_b = KiTS1(train_data_path, "./Datasets/kits", split=f'train_lab{args.labelnum}', patch_size=patch_size)
    lab_loader_b = DataLoader(trainset_lab_b, batch_size=c_batch_size, shuffle=False, num_workers=0, drop_last=True)

    val_dataset = KiTS1(train_data_path, "./Datasets/kits", split=f'test')
    ...
    for epoch_num in iterator:
        ...
        if epoch_num % 10 == 0:
            ...
            # val model 1
            model.eval()
            dice_sample = test_3d_patch.var_all_case_RNCL(args.root_path, model, model2, num_classes=num_classes,
                                                            patch_size=patch_size,
                                                            stride_xy=32, stride_z=32, model_choose=0, dataset='KiTS19',
                                                            fuse_feature1=True)
            ...
            model.eval()
            model2.eval()
            dice_sample2 = test_3d_patch.var_all_case_RNCL(args.root_path, model, model2, num_classes=num_classes,
                                                             patch_size=patch_size,
                                                             stride_xy=32, stride_z=32, model_choose=1,
                                                             dataset='KiTS19', has_feature=True, multi_step=True,
                                                             fuse_feature1=True)
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    ...
    c_batch_size = args.batch_size//2
    trainset_lab_a = KiTS1(train_data_path, "./Datasets/kits",
                           split=f'train_lab{args.labelnum}',
                           patch_size=patch_size)
    lab_loader_a = DataLoader(trainset_lab_a, batch_size=c_batch_size, shuffle=False, num_workers=0, drop_last=True)
    trainset_lab_b = KiTS1(train_data_path, "./Datasets/kits",
                           split=f'train_lab{args.labelnum}',
                           patch_size=patch_size)
    lab_loader_b = DataLoader(trainset_lab_b, batch_size=c_batch_size, shuffle=False, num_workers=0, drop_last=True)
    trainset_unlab_a = KiTS1(train_data_path, "./Datasets/kits",
                             split=f'train_unlab{args.labelnum}',
                             patch_size=patch_size)
    unlab_loader_a = DataLoader(trainset_unlab_a, batch_size=c_batch_size, shuffle=False, num_workers=0, drop_last=True)
    trainset_unlab_b = KiTS1(train_data_path, "./Datasets/kits",
                             split=f'train_unlab{args.labelnum}',
                             patch_size=patch_size)
    ...
    load_net_opt(ema_model1, optimizer, pretrained_model1)
    ...
    for epoch in iterator:
        ...
        for step, ((img_a, lab_a), (img_b, lab_b), (unimg_a, _), (unimg_b, _)) in enumerate(
            ...
            with torch.no_grad():
                ...
                plab_a = get_cut_mask(unoutput_a_1, nms=0)
                plab_b = get_cut_mask(unoutput_b_1, nms=0)
                img_mask, loss_mask = context_mask(img_a, args.mask_ratio)
            ...
            label_all = torch.cat([l_label_batch_ori, ul_label_batch_ori], dim=0)
            loss_model1 = (loss_l + loss_u)
            ...
            true_labels_onehot = torch.unsqueeze(combined_true_labels, 1).float()
            if num_classes == 2:
                true_labels_onehot = torch.cat((1 - true_labels_onehot, true_labels_onehot), dim=1)

            output2, _ = model2(combined_images, coarse_logits,features2)
            output2 = torch.softmax(output2, dim=1)
            loss_ce = F.cross_entropy(output2, true_labels_onehot)
            loss_dice_2 = dice_loss_one_hot(output2, true_labels_onehot)

            lambda_dice = args.lambda_dice
            loss_model2 = loss_ce + lambda_dice * loss_dice_2
        ...
        if epoch % 10 == 0:
            ...
            model1.eval()
            dice_sample = test_3d_patch.var_all_case_guide_model(args.root_path, model1, num_classes=num_classes, patch_size=patch_size,
                                                        stride_xy=32, stride_z=32,dataset='KiTS19',
                                                        )
            ...
            model1.eval()
            model2.eval()
            dice_sample2 = test_3d_patch.var_all_case_RNCL(args.root_path, model1, model2, num_classes=num_classes,
                                                             patch_size=patch_size,
                                                             stride_xy=32, stride_z=32, model_choose=1,
                                                             dataset='KiTS19', has_feature=True, multi_step=True,
                                                             fuse_feature1=True)
            if dice_sample2 > best_dice2:
                ...
                save_mode_path = os.path.join(snapshot_path,
                                              'iter_{}_dice_{}_diffu.txt'.format(iter_num, round(dice_sample2, 4)))
                with open(save_mode_path, 'w')as f:
...
if __name__ == "__main__":
    ...
    pre_snapshot_path = "./experiments/KiTS19/{}/pre_train".format(args.exp)
    self_snapshot_path = "./experiments/KiTS19/{}/self_train".format(args.exp)
    print("Starting Diff-ESSPIL training.")
    for snapshot_path in [pre_snapshot_path, self_snapshot_path]:
        if not os.path.exists(snapshot_path):
            os.makedirs(snapshot_path)
    # copy train files
    shutil.copyfile(__file__, f"./experiments/KiTS19/{args.exp}/{args.exp}.py")
