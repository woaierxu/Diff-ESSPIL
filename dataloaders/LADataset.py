...
class LAHeart(Dataset):
    ...
    def __init__(self, data_dir, list_dir, split, reverse=False, logging=None):
        ...
        if 'train_lab' in split:
            data_path = os.path.join(list_dir,f'{split}.txt')
            self.transform = tr_transform
        elif 'train_unlab' in split:
            data_path = os.path.join(list_dir,f'{split}.txt')
            self.transform = tr_transform
...
class LAHeart_getIDX(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            tr_samples = self.transform(samples)
        image_, label_ = tr_samples
        return idx, image_.float(), label_.long()

class LAHeart_cognition_difference(Dataset):
    """ LA Dataset """
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            tr_samples = self.transform(samples)
        image_, label_ = tr_samples
        return image_.float(), label_.long(),name
