...
# all_flair = []
# for root, dirs, files in os.walk(r'E:\4_Dataset\BraTS\2019\brats2019\MICCAI_BraTS_2019_Data_Training'):
#     for file in files:
#         if file.endswith('_flair.nii.gz'):
#             all_flair.append(os.path.join(root, file))
all_flair = glob.glob("E:/4_Dataset/BraTS/2019/brats2019/MICCAI_BraTS_2019_Data_Training/*/*/*_flair.nii")
out_path =r'E:\4_Dataset\BraTS\2019-SSL'
for p in all_flair:
    ...
    lab[lab > 0] = 1
    # print(numpy.count_nonzero(lab==0))
    # print(numpy.count_nonzero(lab==1))
    # print(numpy.count_nonzero(lab==2))
    # print(numpy.count_nonzero(lab==3))
    uid = p.split("/")[-1].split('\\')[-1].replace('_flair.nii','')
    f = h5py.File(os.path.join(out_path,uid+'.h5'), 'w')
