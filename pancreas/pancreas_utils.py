...
class CutPreMeasures(Measures):
    ...
    def update(self, out, lab, ce_loss, dice_loss, loss, model_choose = 0):
        
       masks = get_mask(out)
       train_dice1 = statistic.dice_ratio(masks, lab)
       model_name = 'VNet' if model_choose==0 else 'DiffUNet'
       self.logger.info(f"\t{model_name} : ce loss: %.4f, dice loss: %.4f, total loss: %.4f, train_dice: %.4f" %
       (ce_loss.item(), dice_loss.item(), loss.item(), train_dice1) )

    def update2(self, out, lab, ce_loss, dice_loss, loss):

        train_dice = 1-dice_loss_one_hot(out, lab)
        self.logger.info("\tDiffU : ce loss: %.4f, dice loss: %.4f, total loss: %.4f, train dice: %.4f" %
                         (ce_loss.item(), dice_loss.item(), loss.item(),train_dice))
...
class CutmixFTMeasures(Measures):
    def __init__(self, writer, logger):
        keys = ['mix_loss_lab', 'mix_loss_unlab']
...
def get_cut_mask_two(out1, out2, thres=0.5, nms=True, connect_mode=1):
