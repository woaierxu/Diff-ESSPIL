...
def get_dataset_path(dataset='pancreas', labelp='10percent'):
    if labelp=='20percent':
        tag = '_per20'
    elif labelp =='10percent':
        tag = '_per10'
    elif labelp == '5percent':
        tag = '_per5'
    files = [f'train_lab{tag}.txt', f'train_unlab{tag}.txt', 'test.txt']
...
class Pancreas(Dataset):
    ...
    def __init__(self, base_dir, name, split, no_crop=False, labelp=20, reverse=False, TTA=False):
        ...
        self.image_list = [self._base_dir + "/{}".format(item.strip().replace('data','')) for item in self.image_list]
...
def get_ema_model_and_dataloader_rncl(data_root, split_name, batch_size, lr, labelp=10):
    print("Initialize ema cutmix: network, optimizer and datasets...by MQX")
    """Net & optimizer"""
    net1 = create_guide_model()
    net2 = create_refinement_model()

    ema_net1 = create_guide_model(ema=True).cuda()
    ...
    return net1, net2, ema_net1, optimizer1, optimizer2, lab_loader_a, lab_loader_b, unlab_loader_a, unlab_loader_b, test_loader