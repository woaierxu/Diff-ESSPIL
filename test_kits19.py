...
test_path = "./experiments_ori/KiTS19/train_kits19_rncl_lab10/self_train"
root_path = r"/data1/mengqingxu/Dataset/KiTS19/KiTS19-SSL"
exp_name = pathlib.Path(test_path).parent.name
num_classes = 2

EXCEL_FILENAME = os.path.join('./experiments/KiTS19', "test_metrics_summary.xlsx")
...
def creat_DiffUnet_Vencoder(infer_start_timestep=0, ema=False):
    timesteps_value = 20
    net = DiffUnet_predMask_FuseFeature(in_channels=1,
                                    num_classes=num_classes_dfu,
...
def create_ResVnet(ema=False):
    net = ResVNet(n_channels=1, n_classes=2, normalization='instancenorm', has_dropout=False)
    net = nn.DataParallel(net)
    model = net.cuda()
    if ema:
        for param in model.parameters():
...
def testKiTS19(infer_start_timestep=0):
    net1 = create_Vnet()
    ...
    time0 = time.time()
    avg_metric2 = test_all_case_DiffUV(net1, net2, num_classes=num_classes,
                                patch_size=(128,128,64), stride_xy=16, stride_z=16,
                                model_choose=1,has_feature=True,multi_step=True,is_KiTS19=True,image_list=image_list,
                                fuse_feature1=True,
                                nms=0)
...
if __name__ == '__main__':
    ...
    for i in range(19):
        ...
        testKiTS19(infer_start_timestep=current_infer_timestep)
